...
public class ImageDao {
    ...
    public void insert(Image image){
        ...
        // 2.创建并拼装 SQL 语句
        String sql="insert into image_table values(null,?,?,?,?,?,?)";  // imageId 是自增主键,不需要赋值
        ...
        try {
            ...
            // 4.关闭连接和statement对象（无论插入是否成功，都需要关闭连接）
    ...
    public List<Image> selectAll(){
        ...
        try {
            // 3.执行 SQL 语句
            statement=connection.prepareStatement(sql);
            resultSet =statement.executeQuery();  //得到查询结果集

            // 4.处理结果集 resultSet （把结果放到我们的 List 列表当中去）
    ...
    public void delete(int imageId){
        ...
        try {
            ...
            int ret=statement.executeUpdate();  // 删除一条记录，那么影响的行数应该为1
            if(ret!=1){
    ...
    /**
     *  按照 md5 的值来查找数据库中的 “图片”Image对象（图片的 md5 值是唯一的）
     * @param md5
     * @return
     */
    public Image selectByMd5(String md5){
        ...
        try {
            // 3.执行 SQL 语句
            statement=connection.prepareStatement(sql);
            statement.setString(1,md5);    //md5 在数据库中是按字符串形式存储的，所以要调用 setString() 方法
    ...
    // 如果你的数据库在云服务器上, 没有在本地服务器上. 这个程序在本地直接运行就会无法访问数据库
